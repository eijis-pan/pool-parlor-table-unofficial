...
public class StandardCaromPhysicsManager : UdonSharpBehaviour
{
#if HT_QUEST
   private const float k_MAX_DELTA = 0.05f; // max time to process per frame on quest (~4)
    ...
    [NonSerialized] public float power_adjust = 1.1f; // 1.0f;
    [NonSerialized] public float gravity_etc_fixed = -0.00073549875f; // 0.00122583125f * 0.6f;
    ...
    private bool updateVelocity(int id, GameObject ball)
    {
        ...
        if (balls_P[id].y < 0.001)
        {
            ...
            if (cvMagnitude <= 0.1f)
            {
                ...
                // (baked):
                V += gravity_etc_fixed * VwithoutY.normalized;
   ...
   _phy_table_init();
    ...
    private void applyPhysics(float pV0)
    {
        float V0 = pV0 * power_adjust;
        GameObject cuetip = table.activeCue._GetCuetip();
